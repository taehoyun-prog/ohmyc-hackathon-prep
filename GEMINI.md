# CLAUDE.md — ohmyc production 협업 규약

이 파일은 ohmyc production 작업에 들어오는 모든 Claude 세션이 가장 먼저 읽는
"운영 매뉴얼"입니다. 컨텍스트 체인이 흔들리지 않게 유지하는 게
...
1. `MEMORY.md`를 먼저 펼친다. 메모리가 깔려 있으면 컨텍스트로
   삼고, 비어 있으면 처음부터 시작해야 하는 상태다.
...
ohmyc production 작업은 다음 사이클 구조로 흐른다. 각 단계는 다음 단계의 입력을
만들어내므로 순서를 건너뛰지 않는다.
...
이전 대화·문서·레포 구조에서 추정한 항목에 확신도(높음/중간/낮음)를
...
### 3단계 — "어떻게 실행·어디서 깨지나"
4단계가 실패하지 않도록 받쳐주는 가드레일 층이다. 다음을 메모리화한다.
- production 가드레일 (Supabase 익명 인증만, 추가 백엔드·추상화·
  디자인 시스템·테스트 인프라 신설 금지) — "우리 약속" 형태로
- 시연 스크립트의 1차 가설 (멘트·화면 동선·종료 트리거)
- 위험 등록부: "조건 → 발현 신호 → plan_b" 3단 형식
- 워크플로우 약속 (Deep Interview → Ralph → Vercel 사이클, 회고 노트
  위치, MEMORY.md auto-append 규약)
- Plaitoon은 이새로찬 CEO 소유 별개 회사. ohmyc는 독립 제품 (Plaitoon 자산 자동 차용 없음). IR 자료에서만 트랙션 (₩132M/월·+113.8%·AI비용 48%→30%) 참조 가능.

### 4단계 — Deep Interview → Ralph (한 단계 안의 두 페이즈)

**4-A. Deep Interview** (`/oh-my-claudecode:deep-interview`)
이전 메모리에서 답이 나오는 건 다시 묻지 않는다. 종착지는 ralph가
그대로 돌릴 수 있는 산출물이다.
- `prd.json`: stories[] (acceptance_criteria는 관찰 가능 단위로) /
  non_goals[] / risks[]+plan_b / dependencies[]
- `demo_script.md`: 시연 멘트 + 화면 동선
- `ralph_handoff.md`: 다음 단계 진입점 1장 + critic 추천(architect /
  critic / codex)과 그 근거

산출물이 메모리(가드레일·위험 등록부)와 어긋나면 4-B로 넘어가지
...
- vercel deploy 성공 + 라이브 URL에서 시연 시나리오 끝까지 무중단
...
production 사이클 1회의 성공은 다음 4가지 동시 충족이다.
1. 시연 시나리오 1개 완성
